...
<div class="w-screen h-screen grid grid-rows-[auto_auto_1fr] grid-cols-[250px_1fr] gap-1 *:border">
  ...
  <div class=" border-b-0 flex justify-center items-center h-full col-span-2">
    <h1 class="text-xl p-2 font-bold">Quizmaker.gg</h1>
  </div>
  ...
  <!--Quiz management-->
  <div class="flex items-center gap-4 p-4 *:btn-primary *:flex *:justify-center">
    <h2 class="">Quizzes</h2>
    <button class="" onclick={addQuiz}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
      </svg>
      <span>Add Quiz</span>
    </button>
    <button class="" onclick={editQuiz}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
      </svg>
      <span>Edit Quiz</span>
    </button>
    <button class="" onclick={deleteQuiz}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
      ...
      {#if userData.ql.length === 0}
        <div class="h-full flex justify-center items-center self-center">
          <div class="flex items-center flex-col">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-1/4 text-dusk-blue-800">
      ...
      {:else}
        <main class="flex flex-wrap gap-4 p-4 overflow-y-scroll">
          {#each userData.ql as quiz, i (quiz)}
            <button class={`quiz border ${i === selectedIndex ? "border-2" : ""}`} onclick={() => selectedIndex = i}>
