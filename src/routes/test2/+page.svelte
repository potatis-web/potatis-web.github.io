<script>
  ...
  function handleZoom(zoomIn) {
    const zoomAmount = zoom.toFixed(2) <= 0.1 ? 0.01 : 0.1;
    zoom = Math.max(0.01, zoom.toFixed() + (zoomIn ? zoomAmount : -zoomAmount));
  }
  ...
  function drawRect(position, size, color = 'gray') {
	...
	function drawBodies() {
		for (let i = 0; i < bodies.length; i++) {
			...
			const relativeX = body.x * zoom + camera.x;
      ...
      body.update();
			...
			if (distanceToMouse(new Vector2(relativeX, relativeY)) <= radius) {
	...
	function drawAddStageUI() {
		...
		if (newBody?.x) {
			...
			drawCircle(newBody.x, newBody.y, newBody?.radius ?? distanceToMouse(new Vector2(newBody.x, newBody.y)), false);
		}
...
<main class="text-white">
  ...
  <div class="fixed bottom-4 left-4 flex items-center gap-2  *:border">
    ...
    <p>Position: {newBody?.x ?? mouse.x}, {newBody?.y ?? mouse.y}</p>
		<p>Radius: {newBody?.radius?.toFixed(1) ?? (newBody.x > 0 ? distanceToMouse(new Vector2(newBody.x, newBody.y)).toFixed(1) : "N/A")}</p>
  ...
  <div class="fixed bottom-4 right-4 flex items-center gap-2  *:border">
    ...
    <button class="cursor-pointer p-4" onclick={() => handleZoom(true)}>+</button>
    <button class="cursor-pointer p-4 " onclick={() => handleZoom(false)}>-</button>
  </div>
</main>