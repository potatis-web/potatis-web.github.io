<script>
	...
	let nameWarningShown = false;
	let questionWarningShown = false;
	...
	$effect(() => {
		...
		if (quiz.name === "" && !nameWarningShown) {
			nameWarningShown = true;
			makeNotification("Warning: Quiz name is empty. Please add one before exiting", "warn");
		} else if (quiz.name !== "" && nameWarningShown) {
			nameWarningShown = false;
		}

		const hasEmptyQuestion = quiz.q?.some((question) => !question?.title?.trim());
		if (hasEmptyQuestion && !questionWarningShown) {
			questionWarningShown = true;
			makeNotification("Warning: One or more questions are empty. Please add question text before exiting", "warn");
		} else if (!hasEmptyQuestion && questionWarningShown) {
			questionWarningShown = false;
		}
	});
	...
	function modalHandler() {
		...
		const handler = (e) => {
			...
			if (e.target === modalbg) {
				console.log("closed")
				modalOpen = false;
				modalbg.removeEventListener("click", handler)
			}
		}
		modalbg.addEventListener("click", handler);
	}
...
{#if quiz}
	...
	{#if modalOpen}
		<div id="modal-bg" onload={modalHandler} class="h-screen w-screen bg-black opacity-10 top-0 left-0 fixed flex justify-center items-center z-10">
			<div class="bg-soft-linen-200 text-dusk-blue-900 w-1/2 h-9/10 z-20 shadow border-soft-linen-300 rounded"></div>
		</div>
	{/if}
	<main class="grid h-screen grid-cols-[300px_1fr] grid-rows-[1fr_275px]">
		<aside class="z-10 row-span-2 flex flex-col items-center gap-4 shadow">
			<div class="grid h-40 w-full grid-cols-2 grid-rows-3 ">
				...
				<div class="row-span-2">
					<!--Image picker-->
					<div class="relative flex h-full w-full items-center justify-center border-soft-linen-200">
				...
				<div class="col-span-2 flex flex-row items-center justify-around p-8 border-b-soft-linen-200 shadow">
			...
			<div class="flex h-full w-full flex-col items-center gap-4 overflow-y-scroll p-4 ">
				{#each quiz.q as question, i (question)}
					<button
						class={`btn-primary w-5/6 p-6 truncate ${question.title?.trim() ? '' : 'text-neutral-500'}`}
						onclick={() => {
...
{:else}
	...
	<main class="flex h-screen items-center justify-center">
		<div class="relative flex flex-col items-center gap-8 *:text-center">
			...
			<a href={resolve("/")} class="btn-primary">Back to dashboard</a>
		</div>
	</main>
...
<!--Notifications-->
<aside class="fixed right-4 bottom-4 flex flex-col gap-4 z-50">
